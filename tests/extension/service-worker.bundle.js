...
var _HistoryServiceWorkerModule = class _HistoryServiceWorkerModule extends REXServiceWorkerModule {
  constructor() {
     ...
     */
    __publicField(this, "urlActiveBuffer", []);
    __publicField(this, "urlActiveUnsubscribe", null);
    /**
   ...
   */
  trySubscribeUrlActive() {
    if (this.urlActiveUnsubscribe !== null) {
      return;
    }
    const seam = getUrlActiveSeam();
    if (seam === void 0 || typeof seam.subscribe !== "function") {
   ...
   */
  findUrlActiveMatch(url, visitTime) {
    if (url.startsWith("CATEGORY:")) {
      return null;
    }
    const tolerance = this.config?.page_events_link_tolerance_ms ?? DEFAULT_PAGE_EVENTS_LINK_TOLERANCE_MS;
    if (tolerance <= 0) {
      return null;
    }
    let best = null;
  ...
  async setup() {
    ...
    globalThis.chrome.storage.onChanged.addListener((changes, areaName) => {
      ...
      if (changes.REXConfiguration || changes.rexIdentifier) {
        this.loadConfigOnly().then(async () => {
   ...
   */
  async loadConfigOnly() {
    await this.loadConfigurationImpl(false);
  }
  async loadConfigurationImpl(syncLists) {
    try {
   ...
   */
  async resolveRecordedUrl(item, context) {
    const url = item.url ?? "";
    ...
    if (domainOnlyResult.filteredByList) {
      ...
      if (!allowCheck.allowed) {
        ...
        recordedTitle = "";
        registeredDomain = "";
        await this.maybeLogFilteredUrlDebug(url, recordedUrl, "NOT_ON_ALLOWLIST", void 0, context);
      } else {
   ...
   */
  async emitCeilingDiagnostic(item, windowStart, windowEnd) {
    try {
   ...
   */
  async emitAggregateRecord(item, windowStart, windowEnd) {
    const resolved = await this.resolveRecordedUrl(item, { history_item_id: item.id });
